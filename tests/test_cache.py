...
def test_memory_cache_writes_are_atomic_and_monotonic() -> None:
    async def scenario() -> None:
        ...
        assert lookup.value == newest
        assert lookup.value is not newest
        ...
        forced_lookup = await cache.get(KEY_A, now=200.0, policy=policy)
        assert forced_lookup.value == older
        assert forced_lookup.value is not older
...
def test_batch_write_selects_newest_duplicate_regardless_of_input_order() -> None:
    async def scenario() -> None:
        ...
        assert first.value == newest
        assert first.value is not newest

        await cache.clear()
        await cache.set_many_if_newer((older, newest))
        second = await cache.get(KEY_A, now=200.0, policy=policy)
        assert second.value == newest
        assert second.value is not newest
